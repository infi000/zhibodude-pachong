...
router.get("/:id", function(req, res) {
    if (req.params.id == "1") {
        ...
        start = setInterval(function() {
            if (get == 1) {
                superagent.get("https://www.espnzhibo.com/live/list")
                    .buffer()
                    // .charset('gb2312')
                    .type("json")
                    .end(function(err, sres) {
                        ...
                        fs.writeFile("./public/data/espn.json", sres.text, function(err) {
                            if (err) {
                                console.error(err);
                            } else {
                                console.log("写入espn.json成功。" + new Date());
                            };
                        });
        ...
        get = 2;
        clearInterval(start);
        console.log("结束写入espn.json"+new Date());
        res.send("结束" + get)
    }
});
